...
/**
 * @fileoverview  Implement a virtual trackball in the armgpu.Trackball
...
// Requires armgpu.Application
// Requires armgpu.DragEvent
// Requires armgpu.Vector3
...
armgpu.Trackball = function() {
   ...
   * @private
   */
  this.rollStart_ = new armgpu.Vector3(0, 0, 1);
 ...
 * @private
 */
armgpu.Trackball.prototype.initInFrame_ = function(frameSize) {
 ...
 * @private
 */
armgpu.Trackball.prototype.convertClientPoint_ = function(clientPoint) {
 ...
 * @return {armgpu.Vector3} the 3D point on the virtual trackball.
 * @private
 */
armgpu.Trackball.prototype.projectOnTrackball_ = function(point) {
  ...
  var trackballPoint = new armgpu.Vector3(point.x, point.y, zValue);
 ...
 */
armgpu.Trackball.prototype.startAtPointInFrame =
 ...
 *     sin(angle/2).
 */
armgpu.Trackball.prototype.rollToPoint = function(dragPoint) {
  var rollTo = this.convertClientPoint_(dragPoint);
  if ((Math.abs(this.rollStart_.x - rollTo.x) <
               armgpu.Trackball.DOUBLE_EPSILON) &&
...
armgpu.Trackball.prototype.doSetCameraOrientation = function(orientationQuat) {
    if (armgpu.setCameraOrientation != undefined) {
        armgpu.setCameraOrientation(orientationQuat);
    }
    else {
        //var elem = document.getElementById('camera-dbg');
        //elem.innerHTML += 'doSetCameraOrientation armgpu.setCameraOrientation == null!<br>';
    }
}
 ...
 * @param {!armgpu.Application} view The view controller that called this
 *     method.
 * @param {!armgpu.DragEvent} dragStartEvent The DRAG_START event that
 *     triggered this handler.
 */

armgpu.Trackball.prototype.handleStartDrag =
...
//  var elem = document.getElementById('camera-dbg');
//  elem.innerHTML += 'handleStartDrag<br>';

  // this.doSetCameraOrientation(this.cameraOrientation_);
 ...
 * @param {!armgpu.Application} view The view controller that called this
 *     method.
 * @param {!armgpu.DragEvent} dragEvent The DRAG event that triggered this
 *     handler.
 */
armgpu.Trackball.prototype.handleDrag =
...
//  controller.setCameraOrientation(
  /*
  this.doSetCameraOrientation(
    armgpu.multQuaternions(this.rollToPoint(flippedY),
 ...
 * @param {!armgpu.Application} view The view controller that called this
 *     method.
 * @param {!armgpu.DragEvent} dragEndEvent The DRAG_END event that triggered
 *     this handler.
 */
armgpu.Trackball.prototype.handleEndDrag =
  ...
  this.cameraOrientation_ = armgpu.multQuaternions(this.rollToPoint(flippedY),
                                                    this.cameraOrientation_);
//  controller.setCameraOrientation(this.cameraOrientation_);
  // this.doSetCameraOrientation(this.cameraOrientation_);
};
 ...
 */
armgpu.multQuaternions = function(q0, q1) {
 ...
 * @type {number}
 */
armgpu.Trackball.DOUBLE_EPSILON = 1.0e-16;
