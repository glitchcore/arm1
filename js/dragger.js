 ...
 * @constructor
 */
armgpu.Dragger = function(opt_target) {
  /**
   * The event target.
   * @type {Element}
   * @private
 ...
 */
armgpu.Dragger.prototype.addDragListener = function(listener) {
 ...
 */
armgpu.Dragger.prototype.onMouseDown = function(event) {
  ...
  var dragStartEvent = { type: armgpu.Dragger.DragEvents.DRAG_START,
 ...
 */
armgpu.Dragger.prototype.onMouseMove = function(event) {
  if (!this.isDragging_)
    return;
  var dragEvent = { type: armgpu.Dragger.DragEvents.DRAG,
                    clientX: event.offsetX,
                    clientY: event.offsetY};
  var i;
  for (i = 0; i < this.listeners_.length; ++i) {
 ...
 */
armgpu.Dragger.prototype.onMouseUp = function(event) {
  ...
  this.boundMouseMove_ = null;
  this.isDragging_ = false;
  var dragEndEvent = { type: armgpu.Dragger.DragEvents.DRAG_END,
                       clientX: event.offsetX,
                       clientY: event.offsetY};
  var i;
  for (i = 0; i < this.listeners_.length; ++i) {
